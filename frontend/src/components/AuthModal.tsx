...
import { loginWithGoogle, loginUser, registerUser, UserProfile, forgotPassword, resetPassword } from '../services/authService'
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (type === 'forgotPassword') {
      if (forgotStep === 1) {
        ...
        if (!email.trim() || !verificationCode.trim() || !forgotNewPassword.trim()) {
          setError('Vui lòng điền đầy đủ các thông tin!')
          return
        }
        setIsLoading(true)
        try {
          const res = await resetPassword(email.trim(), verificationCode.trim(), forgotNewPassword)
          setForgotSuccessMsg(res.message + ' Đang quay lại trang đăng nhập...')
          setTimeout(() => {
            onSwitch('login')
            setForgotStep(1)
            setForgotSuccessMsg(null)
      ...
      >
        ...
        <div className="mb-6">
          <p className="font-poppins text-actionBlue text-sm font-semibold tracking-wider uppercase mb-1">
            {type === 'forgotPassword' ? 'Khôi phục tài khoản' : (isLogin ? 'Welcome back' : 'Get started')}
          </p>
          <h2 className="font-poppins text-brandDark text-2xl font-bold tracking-tight">
            {type === 'forgotPassword' ? 'Quên mật khẩu' : (isLogin ? 'Đăng nhập' : 'Đăng ký tài khoản')}
          </h2>
        </div>
        ...
        <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
          {type === 'forgotPassword' ? (
            <>
              {forgotStep === 1 ? (
                ...
                <>
                  <div className="relative">
                    <Mail size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-secondaryText/60 opacity-55" />
                    <input
                      type="email"
                      readOnly
                      placeholder="Email"
                      value={email}
                      className="w-full bg-offWhite1 border border-grayBorder text-secondaryText/80 cursor-not-allowed rounded-lg py-3.5 pl-10 pr-4 text-sm focus:outline-none"
                    />
                  </div>
                  <div className="relative">
                    ...
                    <input
                      type="text"
                      placeholder="Mã xác thực gửi qua Email"
              ...
              {isLogin && (
                <div className="text-right -mt-1">
                  <button
                    ...
                    className="text-xs text-secondaryText hover:text-actionBlue transition-colors font-medium"
                  >
                    Quên mật khẩu?
                  </button>
                </div>
          ...
          {/* Success Message */}
          {forgotSuccessMsg && (
            <div className="bg-[#EAFDF0] border border-[#2E7D32]/40 rounded-lg p-3 text-[#2E7D32] text-xs text-center font-medium my-1">
              {forgotSuccessMsg}
            </div>
          )}
          ...
          >
            {isLoading ? 'Đang xử lý...' : (type === 'forgotPassword' ? (forgotStep === 1 ? 'Gửi mã khôi phục' : 'Xác nhận đặt lại') : (isLogin ? 'Đăng nhập' : 'Đăng ký'))}
          </button>

          {/* Divider & Google Button – hide on forgot password */}
          {type !== 'forgotPassword' && (
            <>
              {/* Divider */}
